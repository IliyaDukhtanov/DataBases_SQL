DROP DATABASE IF EXISTS Semimar4;
CREATE DATABASE Semimar4;
USE Semimar4;
...
-- Задача 3: 
-- выбрать id друзей пользователя с id = 1(используяUNION)
...
SELECT u.id, u.firstname, u.lastname, pf.hometown, pf.photo_id
FROM users u
JOIN profiles pf
ON u.id = pf.user_id;

-- Задача 5: Список медиафайлов пользователей с количеством лайков (используя JOIN)
SELECT media.filename, COUNT(likes.id) Likes
FROM media
JOIN likes
